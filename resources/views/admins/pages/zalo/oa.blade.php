...
@section('content')
    ...
    <script>
        ...
        connectOaBtn.addEventListener('click', function() {
            ...
            if (oaId) {
                const url = `{{ route('admin.zalo.oa.updateOaStatus', ['oaId' => '__oaId__']) }}`.replace(
                    '__oaId__', oaId);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            updateActiveOaInfo(data.acitveOaName, data.accessToken, data.refreshToken);
        ...
        refreshTokenBtn.addEventListener('click', function() {
            const url = `{{ route('admin.zalo.oa.refreshAccessToken') }}`;
            ...
            fetch(url, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        accessTokenDisplay.textContent = data.new_access_token.slice(0, 20) + '...' + data
                            .new_access_token.slice(-10);
                        accessTokenDisplay.dataset.token = data.new_access_token;

                        // Cập nhật refresh token mới nếu có
                        if (data.new_refresh_token) {
                            refreshTokenDisplay.textContent = data.new_refresh_token.slice(0, 20) + '...' + data
                                .new_refresh_token.slice(-10);
                            refreshTokenDisplay.dataset.token = data.new_refresh_token;
                        }
        ...
        function updateActiveOaInfo(name, accessToken, refreshToken) {
            ...
            if (accessToken) {
                ...
                accessTokenDisplay.textContent = 'N/A';
            }
            ...
            if (refreshToken) {
                ...
                refreshTokenDisplay.textContent = 'N/A';
            }
        }
    </script>
@endsection
